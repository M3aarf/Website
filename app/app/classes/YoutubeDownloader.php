...
class YoutubeDownloader
{
    ...
    {
        preg_match("/^.*((youtu.be\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#\&\?]*).*/i", $url, $matches);
        if (!isset($matches[7])) {
    ...
    {
        try {
            $this->youtubeId = self::getYoutubeId($url);
        } catch (Exception $e) {
            $this->youtubeId = self::getYoutubeId("https://www.youtube.com/watch?v=$url");
        }
    }
    ...
    public function getVideoInfo()
    {
        ...
        if (!preg_match('/ytplayer.config = (\{.+});ytplayer/', $clearContent, $matches)) {
            return 'There is no Video with this id';
        }
        $configData = json_decode($matches[1], true);
        if (!isset($configData['args']['url_encoded_fmt_stream_map'])) {
                       return 'There is no Video with this id';
        }
        ...
        if (!isset($configData['args']['adaptive_fmts'])) {
        ...
        $this->videoInfo = $configData['args'];
        $this->videoInfo['html'] = $clearContent;
        $this->videoInfo['video_id'] = $videoId;
        $jsPath = $configData['assets']['js'];
        $jsPlayerClear = file_get_contents('https://www.youtube.com' . $jsPath);
        if (
            preg_match('/set\("signature",\s*(?:([^(]*).*)\)/', $jsPlayerClear, $matches)
            || preg_match('/\s+(\w+)=function\(\w+\).+return \w+\.join\(""\)}/', $jsPlayerClear, $matches)
        ) {
            $signParserFnName = $matches[1];
            preg_match("/$signParserFnName=(function\(.+;)/", $jsPlayerClear, $matches);
            $signFnBody = $matches[1];
            preg_match("/(\w+)\.\w+\([\w\d,]+\)/", $signFnBody, $matches);
            $helperObjName = $matches[1];
            ...
            $this->videoInfo['calcSignatureSteps'] = $calcRule;
...
JS;
            ...
            $this->videoInfo['jsSignatureGen'] = $calcSignExpression;
        }
        $baseInfo = array();
        $baseInfo['name'] = $this->videoInfo['title'];
        $videoId = $this->videoInfo['video_id'];
        $baseInfo['previewUrl'] = "https://img.youtube.com/vi/$videoId/hqdefault.jpg";
        $html = $this->videoInfo['html'];
        if (preg_match('/<p id="eow-description"[^>]+>(.+)<\/p>/', $html, $matches)) {
            $baseInfo['description'] = strip_tags(str_replace('<br />', "\n", $matches[1]));
        }
        $this->videoInfo['baseInfo'] = $baseInfo;
        return $this->videoInfo;
    }

    public function getItagMap()
    {
        $info = $this->getVideoInfo();
        $res = array();
        foreach (array_merge($info['url_encoded_fmt_stream_map'], $info['adaptive_fmts']) AS $fmt) {
            $itag = (int)$fmt['itag'];
            $res[$itag] = $fmt;
        }
        return $res;
    ...
    public static function getItagInfo()
    {
        return array(
            ...
            278 => array('format' => 'webm', 'withVideo' => true, 'withAudio' => false),
            249 => array('format' => 'webm', 'withVideo' => false, 'withAudio' => true),
            250 => array('format' => 'webm', 'withVideo' => false, 'withAudio' => true),
            251 => array('format' => 'webm', 'withVideo' => false, 'withAudio' => true),
        );
    }
    ...
    {
        $fullInfo = $this->getVideoInfo();
        $title = $fullInfo['title'];
        $url = $fmtsItem['url'] . '&title=' . urlencode($title);
        if (isset($fullInfo['calcSignatureSteps'])) {
            $signature = false;
...
//        $downloadInfo[] = $downloadInfo;
    ...
    public function downloadViaServer($itag, $exceptionForError = false)
    {
        $fullInfo = $this->getVideoInfo();
        $fmts = $fullInfo['url_encoded_fmt_stream_map'];
        $fmts = array_merge($fmts, $fullInfo['adaptive_fmts']);
        ...
        if ($exceptionForError) {
            throw new Exception('itag not found');
        }
